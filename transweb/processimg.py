...
def process_image(path, out_path):
    ...
    if img_estim(bytesim,127) == 'dark':
        ret,bytesim = cv2.threshold(np.asarray(text_im),127,255,cv2.THRESH_BINARY_INV)
    text_im = Image.fromarray(bytesim)
    text_im.save(out_path)
    print('%s -> %s' % (path, out_path))

def process_image_data(orig_im):

    scale, im = downscale_image(orig_im)
    datas = cv2.resize(np.asarray(im), None, fx=1.5, fy=1.5, interpolation=cv2.INTER_CUBIC)
    datas = cv2.cvtColor(datas, cv2.COLOR_BGR2GRAY)
    # ret,bytesim = cv2.threshold(np.asarray(bytesim),127,255,cv2.THRESH_BINARY)
    if img_estim(datas,127) == 'dark':
        ret,datas = cv2.threshold(datas,127,255,cv2.THRESH_BINARY_INV)
    orig_im = Image.fromarray(datas)
    edges = cv2.Canny(datas, 100, 200)
...
#         except Exception as e:
#             print '%s %s' % (path, e)
# process_image('abc.png','dest.jpg')